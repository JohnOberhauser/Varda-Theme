...
run() {
    eww -c $HOME/.config/hypr/components/eww open audio_ctl
}

# Open widgets
if [[ ! -f "$LOCK_FILE_AUDIO" ]]; then
    #eww -c $HOME/.config/hypr/components/eww close system calendar music
    touch "$LOCK_FILE_AUDIO"
    run && echo "ok good!"
else
    eww -c $HOME/.config/hypr/components/eww close audio_ctl

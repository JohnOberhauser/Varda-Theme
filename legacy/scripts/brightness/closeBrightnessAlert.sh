...
do
    ...
    alertTimeStamp=$(eww -c $HOME/.config/hypr/components/eww get brightnessAlertLastUpdateTimeStamp)
...
#    echo $(eww -c $HOME/.config/hypr/components/eww get volumeAlertPercent)
    ...
    if [[ $alertTimeStamp -le $(expr $currentTime - $timeoutInMilliseconds) ]]; then
        eww -c $HOME/.config/hypr/components/eww close brightnessAlertWindow
        rm "$LOCK_FILE"
        exit
    fi
